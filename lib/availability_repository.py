...
class AvailabilityRepository():
    ...
    def create(self, availability):
        if self.is_period_overlapping(availability):
            return False

        self._connection.execute(
            'INSERT INTO availabilities(start_date,end_date, space_id) VALUES(%s,%s,%s)',
            [availability.start_date, availability.end_date, availability.space_id]
        )
        return True
    def update(self, availability):
        self._connection.execute(
            'UPDATE availabilities SET start_date = %s, end_date = %s , space_id = %s WHERE id = %s',
            [availability.start_date, availability.end_date, availability.space_id, availability.id]
        )
        return None
    ...
    def is_period_overlapping(self, new_availability):
        existing_periods = self.find_by_space_id(new_availability.space_id)

        for period in existing_periods:
            if (new_availability.start_date <= period.end_date and 
                new_availability.end_date >= period.start_date):
                return True
