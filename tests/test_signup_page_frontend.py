...
def test_page_title_and_main_heading(page, test_web_address):
    ...
    main_heading = page.get_by_role("heading", name="Welcome to the Neighbourhood")
...
def test_login_form_elements(page, test_web_address):
    ...
    expect(name_input).to_have_attribute("name", "name")
    ...
    sign_up_button = page.get_by_role("button", name="Sign-up")
    expect(sign_up_button).to_be_visible()
    expect(sign_up_button).to_have_attribute("type", "submit")
